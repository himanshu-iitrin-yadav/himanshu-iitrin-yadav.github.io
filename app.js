document.addEventListener('DOMContentLoaded', () =>{
    ...
    //adding div to main grid appendChild
    for(var i =0; i<200; i++){
        let child = document.createElement('div');
        grid.appendChild(child);
    }
    for(var i =0; i<10;i++){
        let takenChild = document.createElement('div');
        takenChild.className = "taken";
        grid.appendChild(takenChild);
    }
  ...
  for(var i =0; i<16;i++){
      ...
      miniGrid.appendChild(displayChild);
  }
  ...
  function addScore() {
    for (let i = 0; i < 199; i +=width) {
      ...
      if(row.every(index => squares[index].classList.contains('taken'))) {
        ...
        const squaresRemoved = squares.splice(i, width)//will return removed divs
